...
struct Point
{
    ...
    Point operator -(const Point &b)const{return Point(x-b.x,y-b.y);}//还没测过正确性
    Point operator +(const Point &b)const{return Point(x+b.x,y+b.y);}
    bool friend operator < (Point a, Point b) { return a.x < b.x || (a.x == b.x && a.y < b.y); }
    bool friend operator == (Point a, Point b) { return (a.x - b.x) == 0 && (a.y - b.y) == 0; }
};
...
inline void getLineGeneralEquation(Point p1, Point p2, double& a, double& b, double &c) ///已验证
{
    a = p2.y - p1.y;
    b = p1.x - p2.x;
    c = -a * p1.x - b * p1.y;
...
double DisOnCircle(Point a, Point b, Circle C) ///已验证
{
